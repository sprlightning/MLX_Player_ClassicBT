 ...
 * @copyright GPLv3
 * modified by MLX on 2025-10-23, added support of bitrate(this file use bit_rate)（其实WAV已经有比特率的定义byte_rate）.
 *
 */
struct WAVAudioInfo : AudioInfo {
  ...
  WAVAudioInfo(const AudioInfo &from) {
    ...
    bit_rate = from.bit_rate; // 新增存储比特率（WAV用的就是AudioTypes.h，需要确保在AudioTypes.h添加bit_rate变量的定义）
  }
...
class WAVDecoder : public AudioDecoder {
 ...
 protected:
  ...
  int decodeHeader(uint8_t *in_ptr, size_t in_size) {
    ...
    LOGI("WAV bit_rate: %d", (int)header.audioInfo().bit_rate); // 新增比特率打印
...
class WAVEncoder : public AudioEncoder {
 public:
  ...
  WAVAudioInfo defaultConfig() {
    ...
    info.bit_rate = DEFAULT_SAMPLE_RATE * DEFAULT_CHANNELS * DEFAULT_BITS_PER_SAMPLE; // 新增比特率计算
  ...
  virtual void setAudioInfo(AudioInfo from) override {
    ...
    wav_info.bit_rate = from.bit_rate; // 新增更新比特率（WAV用的就是AudioTypes.h，需要确保在AudioTypes.h添加bit_rate变量的定义）
  ...
  virtual void setAudioInfo(WAVAudioInfo ai) {
    ...
    wav_info.bit_rate = wav_info.byte_rate * 8; // 将计算得到的比特率值从默认的byte_rate变量传递给对外的bit_rate变量
