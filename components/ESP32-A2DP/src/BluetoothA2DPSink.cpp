...
/**
 * 更新于：2025年10月7日，by MLX
 * 修改了获取音频配置的回调函数，增加了编码类型的返回值
 */
...
void BluetoothA2DPSink::handle_audio_cfg(uint16_t event, void *p_param) {
  ...
  // 从A2DP获取codec配置，sr是sample_rate，bps是bits_per_sample，ch是channels，增加了编码返回值codec_type
  get_codec_config(a2d, &sr, &bps, &ch, &codec_type);
  ...
  if (codec_config_callback != nullptr) {
    codec_config_callback(sr, bps, ch, codec_name);
  }
