 ...
 public:
  ...
  AudioInfo audioInfo() override {
    ...
    if (i.nChans != 0 && i.samprate != 0 && i.bitsPerSample != 0) {
      ...
      baseInfo.bit_rate = i.bitrate; // 新增存储比特率（需要确保在mp3dec.h的_MP3FrameInfo结构体有定义，同时AudioTypes.h添加bit_rate变量的定义）
    }
    return baseInfo;
  }
  ...
  static void infoCallback(MP3FrameInfo &i, void *ref) {
    ...
    if (p_helix != nullptr) {
      ...
      baseInfo.sample_rate = i.samprate;
      baseInfo.bits_per_sample = i.bitsPerSample;
      baseInfo.bit_rate = i.bitrate;  // 新增传递比特率（需要确保在mp3dec.h的_MP3FrameInfo结构体有定义，同时AudioTypes.h添加bit_rate变量的定义）
