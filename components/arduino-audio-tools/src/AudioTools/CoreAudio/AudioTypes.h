...
/// @ingroup basic
/// modified by MLX on 2025-10-23, added support of bitrate(this file use bit_rate).
...
struct AudioInfo {
  ...
  /// 比特率bit rate: e.g 320000
  uint32_t bit_rate = 0; // 新增比特率成员变量，推荐用uint32_t类型，初值必须是0，不然无法解析
  /// 总样本数total samples
  uint32_t total_samples = 0; // 新增存储总样本数变量
  ...
  bool operator==(AudioInfo alt) {
    return sample_rate == alt.sample_rate && channels == alt.channels &&
           bits_per_sample == alt.bits_per_sample && bit_rate == alt.bit_rate; // 新增比特率对比（非常关键，不然会解析失败）
  }
  ...
  void set(AudioInfo info) {
    ...
    bit_rate = info.bit_rate; // 新增比特率复制
  }
  ...
  virtual void logInfo(const char* source = "") {
    LOGI("%s sample_rate: %d / channels: %d / bits_per_sample: %d / bit_rate: %d", source,
         (int)sample_rate, (int)channels, (int)bits_per_sample, (int)bit_rate);
  }
};
