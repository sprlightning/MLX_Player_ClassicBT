 ...
 * @copyright GPLv3
 * modified by MLX on 2025-10-24, exposed i2s handle.
 */
class I2SDriverESP32V1 {
 public:
  ...
  /// 暴露I2S通道句柄
  i2s_chan_handle_t getTxHandle() { return tx_chan; }
  i2s_chan_handle_t getRxHandle() { return rx_chan; }
  ...
  struct DriverI2S : public DriverCommon {
   ...
   protected:
    ...
    i2s_chan_config_t getChannelConfig(I2SConfigESP32V1 &cfg) {
      ...
      frame_size = (frame_size == 0) ? 1 : frame_size; // test
