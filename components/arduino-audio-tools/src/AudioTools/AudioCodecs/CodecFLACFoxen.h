...
class FLACDecoderFoxen : public AudioDecoder {
 ...
 protected:
  ...
  void processMetadata() {
    ...
    // 新增获取总样本数
    uint32_t total_samples = fx_flac_get_streaminfo(flac, FLAC_KEY_MAX_BLOCK_SIZE);  // FLAC_KEY_MAX_BLOCK_SIZE ?不知道用什么，详见foxen-flac.h
    LOGI("总样本数: %lu", total_samples);
    ...
    info.total_samples = total_samples; // 新增存储总样本数到AudioInfo（需在AudioInfo中添加字段）
    info.logInfo();
