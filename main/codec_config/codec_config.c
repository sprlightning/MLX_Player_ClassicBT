    ...
    if (a2d->audio_cfg.mcc.type == ESP_A2D_MCT_SBC) {
        ...
        *codec_type = CODEC_AAC; // 更新A2DP编码类型为AAC
                   ...
                   codec_id == A2DP_LHDCV5_CODEC_ID)
        {
            ...
            /* CIE 实证打印（11B: vendor4+codecID2+sr1+bitrate1+ver/framelen1+feat1+raw1）：
             * 帧长位[8]&0x10=0x10→5ms（v5 仅 5ms 帧）；features[9]: 0x80=LLESS48K
             * 0x40=LL 0x20=LLESS24BIT 0x10=LLESS96K 0x04=META 0x02=JAS 0x01=AR */
            ESP_LOGI(CODEC_CONFIG_TAG,
                     "%s: CIE[11] %02X %02X %02X %02X | %02X %02X | %02X | %02X | ver%u framelen%02X | feat%02X | raw%02X",
                     __func__,
                     a2d->audio_cfg.mcc.cie.lhdcv5[0], a2d->audio_cfg.mcc.cie.lhdcv5[1],
                     a2d->audio_cfg.mcc.cie.lhdcv5[2], a2d->audio_cfg.mcc.cie.lhdcv5[3],
                     a2d->audio_cfg.mcc.cie.lhdcv5[4], a2d->audio_cfg.mcc.cie.lhdcv5[5],
                     a2d->audio_cfg.mcc.cie.lhdcv5[6], a2d->audio_cfg.mcc.cie.lhdcv5[7],
                     a2d->audio_cfg.mcc.cie.lhdcv5[8] & 0x0F,   /* A2DP_LHDCV5_VERSION_MASK */
                     a2d->audio_cfg.mcc.cie.lhdcv5[8] & 0x10,   /* A2DP_LHDCV5_FRAME_LEN_MASK=5MS */
                     a2d->audio_cfg.mcc.cie.lhdcv5[9],
                     a2d->audio_cfg.mcc.cie.lhdcv5[10]);
            bits_per_sample = 32; // LHDC supports 16, 24, 32 bits
            /* Must use 32bit to contain 24bit data, if not, will go error in i2s_std_set_clock: 
             * "The 'mclk_multiple' should be the multiple of 3 while using 24-bit data width", 
             * and I2s Write will be very slow, Bluetooth will Pkt dropped. */
