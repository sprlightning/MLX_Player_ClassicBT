 ...
 public:
  ...
  virtual void insert(int pos, const char* str) {
    if (!isConst()) {
      ...
      // strncpy(chars + pos, str, insert_len); // 末尾没有空字符，会导致缓冲区存在溢出风险
      std::string temp(str, insert_len);
      memcpy(chars + pos, temp.c_str(), insert_len); // 更安全的方法
      chars[pos + insert_len] = '\0';
    }
  }
